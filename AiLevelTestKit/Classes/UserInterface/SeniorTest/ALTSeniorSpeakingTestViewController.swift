//
//  ALTSeniorSpeakingTestViewController.swift
//  AILevelTest
//
...
class ALTSeniorSpeakingTestViewController: ALLTSeniorTestBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let lang = LevelTestManager.manager.examInfo?.userLanguage ?? ""
        let quizOrder = testData.quiz?.quizOrder ?? 0
        
        var assetUrl = RequestUrl.AWS +  "/voice/leveltest/\(examSrl)/level\(level)-\(folder)/\(lang)/\(quizOrder).mp3"
...
extension ALTSeniorSpeakingTestViewController: ALTSpeechToTextManagerDelegate {
