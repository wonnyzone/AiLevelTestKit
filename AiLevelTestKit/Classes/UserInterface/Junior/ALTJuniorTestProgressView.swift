//
//  ALTJuniorTestProgressView.swift
//  AILevelTest
//
...
class ALTJuniorTestProgressView: UIView {
...
//        _progressView.backgroundColor = ColourKit.Code.Static.HexF44F69
    ...
    func reloadData() {
        ...
        for i in 0 ..< LevelTestManager.manager.categories.count {
            ...
            label.widthAnchor.constraint(equalTo: _contentView.widthAnchor, multiplier: CGFloat(category.numberOfQuizs) / CGFloat(_totalQuizCount)).isActive = true
            
            guard i < LevelTestManager.manager.categories.count - 2 else { continue }
    ...
    func setStep(_ step: Int, animated: Bool = false) {
        DispatchQueue.main.async { [weak self] in
            ...
            let unit = ((self?.bounds.size.width ?? 0) - 40.optimized) / CGFloat(self?._totalQuizCount ?? 0)
            
            let animations = {[weak self] () -> Void in
                var width = 20.optimized
                width += CGFloat(step) * unit
